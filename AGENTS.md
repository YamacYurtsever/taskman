...
`NextEventPill.tsx` (`client/src/components/Topbar/`) fetches independently of `useAppData`/`App.tsx` (self-contained, no props): polls every 60s, refetches on tab focus (`visibilitychange`), and schedules a one-off `setTimeout` fired exactly at the event's `endIso` so the pill advances to whatever's next the moment the current one ends — not `startIso`, since the event stays "the next event" for the frontend's own purposes while it's in progress. The pill cycles through three display states purely from `startIso`/`endIso` vs. the current time (no extra server round-trip): the `startTime`–`endTime` interval by default (accent-colored), a live `"in 42 min"` countdown once under the 1-hour-to-start threshold, and `"Active"` (pink, matching the `due-today` color) for the window between start and end. A 30s tick re-renders this derived state without re-fetching.
