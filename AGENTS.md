...
`fetch_next_event` also derives `dayLabel` from `date` vs. `today_in_timezone(tz_name)`: `null` for today (no indication needed), `"Tmr"` for tomorrow, a 3-letter weekday (`"Wed"`) for 2–6 days out, and a `"Mon D"`-style full date beyond that. `NextEventPill` prepends it (plus a trailing space) to whichever text is on the time line — the `start – end` interval or the countdown — except while `isActive`, since an in-progress event is always today by construction (`dayLabel` would be `null` there anyway).
