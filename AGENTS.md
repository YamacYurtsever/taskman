...
- [x] Panel is manually resizable by dragging its left border above the narrow breakpoint: clamped to a `360px` minimum, snaps to full-width past ~85% of the available space (matching the narrow-breakpoint cover behavior), manual width persisted in `localStorage` and discarded once the viewport crosses into the narrow breakpoint
...
`GET /api/next-event` (`server/api.py`) queries every calendar in the user's effective calendar list (`auth.effective_calendar_ids` — explicit `calendars` config, else the first 5 of `fetch_user_calendars`, matching `build_calendar_url`'s own fallback) via `events().list(singleEvents=True, orderBy="startTime", maxResults=10)`, filters out all-day events entirely (a small per-calendar lookahead so a calendar whose next couple of entries are all-day, e.g. a holiday, doesn't get skipped outright), and picks the earliest *timed* event across all of them, returning `{title, startIso, endIso, startTime, endTime}` — `startTime`/`endTime` are pre-localized 24-hour `"15.00"`-style strings (server owns timezone math, per the rest of the app), `startIso`/`endIso` are raw UTC timestamps the frontend uses for countdown/active-state math. Any `RefreshError` (dead/revoked token), missing refresh token, or no upcoming timed events resolves to `{"event": null}` rather than an error, so the pill just doesn't render.

`NextEventPill.tsx` (`client/src/components/Topbar/`) fetches independently of `useAppData`/`App.tsx` (self-contained, no props): polls every 60s, refetches on tab focus (`visibilitychange`), and schedules a one-off `setTimeout` fired exactly at the event's `endIso` so the pill advances to whatever's next the moment the current one ends — not `startIso`, since the event stays "the next event" for the frontend's own purposes while it's in progress. The pill cycles through three display states purely from `startIso`/`endIso` vs. the current time (no extra server round-trip): the `startTime`–`endTime` interval by default (accent-colored), a live countdown once under the 1-hour-to-start threshold, and an active indicator (pink, matching the `due-today` color) for the window between start and end. A 30s tick re-renders this derived state without re-fetching.
...
`fetch_next_event` also returns the event's local `date` (`YYYY-MM-DD`, via `local_date_from_storage`), which `NextEventPill` passes to an `onOpenDay` prop when clicked (the pill is a `<button>`). `App.tsx` wires that through `Topbar`'s `onOpenDayCalendar` into the same `sidePanel` state `TaskPanel`/`InfoPanel` already share, as a third variant (`{ type: 'day-calendar'; date }`) — so it reuses the one persistent `Panel` shell rather than mounting a separate one; `DayCalendarPanelHeader` is a static "Schedule" title, not date-derived. `DayCalendarPanel.tsx` (`components/Panel/`) swaps the existing `calendarUrl`'s `mode=WEEK` for `mode=AGENDA` (labeled "Schedule" in Google Calendar's own UI — same thing, the embed API just kept the old name) and appends a `dates=<date>/<date+1>` range. Google's embed API has no true single-day grid mode — `AGENDA` scoped to one real day is the closest available equivalent, rendering as a scrolling event list rather than an hourly grid. A *zero-length* range (`date/date`, tried first) renders a blank iframe instead of an error, hence the `+1` day.

Candidates more than `MAX_DAYS_AWAY` (6) days out are dropped entirely during collection in `fetch_next_event` — the pill never surfaces anything beyond the coming week, so a sparse calendar with nothing sooner just shows no pill rather than jumping to something a month out. Within that window, `fetch_next_event` derives `dayLabel` from `date` vs. `today_in_timezone(tz_name)`: `null` for today (no indication needed), `"Tmr"` for tomorrow, a 3-letter weekday (`"Wed"`) for 2–6 days out — no full-date fallback needed since farther events are filtered out before this point. `NextEventPill` prepends it (plus a trailing space) to whichever text is on the time line — the `start – end` interval or the countdown — except while `isActive`, since an in-progress event is always today by construction (`dayLabel` would be `null` there anyway).
